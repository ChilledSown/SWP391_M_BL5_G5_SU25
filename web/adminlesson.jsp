    ...
    <div class="dashboard-container">
        ...
        <main class="main-content">
            <header class="main-header">
                <h1>Lessons for Course: <c:out value="${selectedCourse.title}" /></h1>
            </header>
            ...
            <div class="header-controls">
                <a href="admincourse?topicId=${topicId}"><button class="back-btn">Back to Courses</button></a>
                <div class="search-section">
                    <form action="adminlesson" method="get">
                        <input type="hidden" name="courseId" value="${selectedCourse.course_id}">
                        <input type="hidden" name="topicId" value="${topicId}">
                        <input type="text" name="query" placeholder="Search lessons by title..." value="${searchQuery}">
                        <button type="submit" class="search-btn" title="Search"><i class="fas fa-search"></i></button>
                        <a href="adminlesson?courseId=${selectedCourse.course_id}&topicId=${topicId}"><button type="button" class="reset-btn" title="Reset"><i class="fas fa-rotate"></i></button></a>
                    </form>
                </div>
            </div>
            <c:choose>
                <c:when test="${empty lessons}">
                    <div class="no-data">No lessons found for this course</div>
                </c:when>
                <c:otherwise>
            ...
            <div class="pagination">
                <c:if test="${currentPage > 1}">
                    <a href="adminlesson?courseId=${selectedCourse.course_id}&topicId=${topicId}&page=${currentPage - 1}&query=${searchQuery}">Previous</a>
                </c:if>
                <c:if test="${currentPage <= 1}">
                    <a class="disabled">Previous</a>
                </c:if>
                <c:forEach begin="1" end="${totalPages}" var="i">
                    <a href="adminlesson?courseId=${selectedCourse.course_id}&topicId=${topicId}&page=${i}&query=${searchQuery}" <c:if test="${currentPage == i}">class="active"</c:if>>${i}</a>
                </c:forEach>
                <c:if test="${currentPage < totalPages}">
                    <a href="adminlesson?courseId=${selectedCourse.course_id}&topicId=${topicId}&page=${currentPage + 1}&query=${searchQuery}">Next</a>
                </c:if>
                <c:if test="${currentPage >= totalPages}">
