    ...
    <div class="dashboard-container">
        ...
        <main class="main-content">
            <header class="main-header">
                <h1>Courses for Topic: <c:out value="${selectedTopic.name}" /></h1>
            </header>
            ...
            <div class="header-controls">
                <a href="admintopic"><button class="back-btn">Back to Topics</button></a>
                <div class="search-section">
                    <form action="admincourse" method="get">
                        <input type="hidden" name="topicId" value="${selectedTopic.topic_id}">
                        <input type="text" name="query" placeholder="Search courses by title..." value="${searchQuery}">
                        <button type="submit" class="search-btn" title="Search"><i class="fas fa-search"></i></button>
                        <a href="admincourse?topicId=${selectedTopic.topic_id}"><button type="button" class="reset-btn" title="Reset"><i class="fas fa-rotate"></i></button></a>
                    </form>
                </div>
            </div>
            <c:choose>
                <c:when test="${empty courses}">
                    <div class="no-data">No courses found for this topic</div>
                </c:when>
                <c:otherwise>
            ...
            <div class="pagination">
                <c:if test="${currentPage > 1}">
                    <a href="admincourse?topicId=${selectedTopic.topic_id}&page=${currentPage - 1}&query=${searchQuery}">Previous</a>
                </c:if>
                <c:if test="${currentPage <= 1}">
                    <a class="disabled">Previous</a>
                </c:if>
                <c:forEach begin="1" end="${totalPages}" var="i">
                    <a href="admincourse?topicId=${selectedTopic.topic_id}&page=${i}&query=${searchQuery}" <c:if test="${currentPage == i}">class="active"</c:if>>${i}</a>
                </c:forEach>
                <c:if test="${currentPage < totalPages}">
                    <a href="admincourse?topicId=${selectedTopic.topic_id}&page=${currentPage + 1}&query=${searchQuery}">Next</a>
                </c:if>
                <c:if test="${currentPage >= totalPages}">
